...
source $(dirname $0)/common.sh || echo "$0: Failed to service common.sh"
...
test -z ${ARGS[--aws_account_id]} && ARGS[--aws_account_id]=${AWS_ACCOUNT_NUMBER}
test -z ${ARGS[--aws_region]} && ARGS[--aws_region]=${4:-"eu-west-1"}
test -z ${ARGS[--ecr_endpoint]} && ARGS[--ecr_endpoint]=${5:-${AWS_ACCOUNT_NUMBER}.dkr.ecr.eu-west-1.amazonaws.com}
