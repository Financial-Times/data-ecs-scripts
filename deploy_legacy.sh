...
test -z ${ARGS[--aws_role]} && ARGS[--aws_role]=${14:-"FTApplicationRoleFor_ingesters"}
...
deploy_cluster() {
    ...
    if [[ $(aws ecs update-service --cluster ${ARGS[--cluster_name]} \
                --service ${ARGS[--ecs_service]}-${ARGS[--suffix]} \
